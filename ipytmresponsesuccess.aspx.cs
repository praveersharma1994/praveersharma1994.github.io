...
public partial class ipytmresponsesuccess : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                Session["ordId"] = Request.QueryString["orderno"].ToString();
            }

            //Session["ordId"] = "FFA0035";
    ...
    void sendEmail(string cart, string orderno, decimal amt, string shippingamt, string cur, string discamt)
    {
        ...
        if (billingdetail != null)
        {
            ...
            try
            {
                string responseq = sm.SendMail(billingdetail.Name, billingdetail.Email, billingdetail.scontact, "Your Order (Success) at S Style Factory has been placed successfully.", body, "Client");
                string sendmailtoadmin = sm.SendMail(billingdetail.Name, billingdetail.Email, billingdetail.scontact, "Order on S Style Factory (Success)", body, "Admin");
            }
            catch (Exception ex)
            {
                Common.LogError(ex);
