...
public partial class Admin_testimg : System.Web.UI.Page
{
    ...
    {
        DataTable dt = DataAccess.GetDataTable("select id,skuname,image from productmaster where Image!='' and Image is not null", CommandType.Text);
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            for (int k = 0; k < dt.Rows.Count; k++)
            {
                ...
                if (File.Exists(Server.MapPath("~/upload/products/temp2/small/" + dt.Rows[k]["image"].ToString())))
                {
                ...
                if (File.Exists(Server.MapPath("~/upload/products/temp2/large/" + dt.Rows[k]["image"].ToString())))
                {
